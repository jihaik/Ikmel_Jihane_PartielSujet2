...
object Part2 {
  ...
  // Create a model for the following categories [salary, purchase, check deposit, check payment, withdrawal]
      ...
      case class Transaction(amount: Double,date:String)

      trait categories

      object categories {

        final case object Amout extends categories

        final case object salary extends categories

        final case object purchase extends categories

        final case object check_deposit extends categories

        final case object check_payment extends categories

        final case object withdrawal extends categories
